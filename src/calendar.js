...
// Add function to show My Plants modal
async function showMyPlantsModal() {
  ...
  try {
    const db = await openDB('gardening-calendar');
    const plantings = await db.getAll('plantings');
    ...
    if (activePlantings.length === 0) {
      plantsHtml = `
        <div class="text-center py-8 text-gray-500 dark:text-gray-400">
          ...
          <p>${t('plants.no_active')}</p>
        </div>
      `;
    } else {
      plantsHtml = activePlantings.map(planting => {
        const displayName = planting.displayName || planting.plantName;
        const customNameInfo = planting.customName ? `<span class="text-xs text-gray-500 dark:text-gray-400">${planting.customName}</span>` : '';
        const startDate = new Date(planting.startDate).toLocaleDateString();
        const completionDate = new Date(planting.completionDate).toLocaleDateString();
        ...
        return `
          <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
            <div class="flex items-start justify-between mb-2">
              ...
              <button onclick="deleteActivePlanting(${planting.id})" class="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300" title="${t('btn.delete')}">
                <i class="fas fa-trash text-sm"></i>
              </button>
            </div>
            
            <div class="space-y-1 text-sm text-gray-600 dark:text-gray-400">
              <div><strong>${t('plants.plant_type')}:</strong> ${planting.plantName}</div>
              <div><strong>${t('plants.location')}:</strong> ${planting.location}</div>
              <div><strong>${t('plants.started')}:</strong> ${startDate}</div>
              <div><strong>${t('plants.current_phase')}:</strong> ${planting.currentPhase}</div>
              <div><strong>${t('plants.expected_completion')}:</strong> ${completionDate}</div>
            </div>
            
            <div class="flex space-x-2 mt-3">
              <button onclick="showPlantDetails(${planting.id})" class="flex-1 px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-sm">
                <i class="fas fa-info-circle mr-1"></i> ${t('btn.view_details')}
              </button>
              <button onclick="addPlantNote(${planting.id})" class="flex-1 px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600 text-sm">
                <i class="fas fa-sticky-note mr-1"></i> ${t('btn.add_note')}
              </button>
            </div>
          </div>
        `;
    ...
    modal.innerHTML = `
      <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-xl font-bold text-gray-900 dark:text-white">${t('plants.my_active_plants')}</h2>
          <button onclick="this.closest('.fixed').remove()" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
            <i class="fas fa-times text-xl"></i>
          </button>
        </div>
        ...
        <div class="flex justify-end mt-6">
          <button onclick="this.closest('.fixed').remove()" class="px-4 py-2 bg-gray-300 dark:bg-gray-600 text-gray-700 dark:text-gray-200 rounded hover:bg-gray-400 dark:hover:bg-gray-500">
            ${t('btn.close')}
          </button>
        </div>
      </div>
    ...
    console.error('Error loading active plants:', error);
    modal.innerHTML = `
      <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full">
        <h2 class="text-xl font-bold text-red-600 mb-4">${t('error.title')}</h2>
        <p class="text-gray-600 dark:text-gray-400 mb-4">${t('error.loading_plants')}</p>
        <button onclick="this.closest('.fixed').remove()" class="px-4 py-2 bg-gray-300 dark:bg-gray-600 text-gray-700 dark:text-gray-200 rounded hover:bg-gray-400 dark:hover:bg-gray-500">
          ${t('btn.close')}
        </button>
      </div>
